...
- Todos os pedidos analisados são de 2019
        ...
        ggplot2::ggplot() +
          ggplot2::geom_line(ggplot2::aes(x = MES_REF, y = Quantidade, group = TIPO_PAGAMENTO, color = TIPO_PAGAMENTO)) +
            ggplot2::scale_color_manual(name = "Tipo de Pagamento", 
                          values = c(cores$azul_escuro, cores$azul_claro, cores$verde_bebe, cores$verde_claro)) +
              ggplot2::scale_x_date(date_labels = "%b", date_breaks = "1 month") +
                tema_semest_linha()) %>% 
  plotly::ggplotly() %>% plotly::layout(heigh = 150)
      ...
      dplyr::summarise(
        ...
        ggplot2::ggplot() +
          ggplot2::geom_line(ggplot2::aes(x = MES_REF, y = Quantidade, group = TURNO, color = TURNO)) +
            ggplot2::scale_color_manual(name = "Turno da entrega", 
                          values = c(cores$azul_escuro, cores$azul_claro, cores$verde_bebe, cores$verde_claro)) +
              ggplot2::scale_x_date(date_labels = "%b", date_breaks = "1 month") +
                tema_semest_linha()) %>% 
  plotly::ggplotly() %>% plotly::layout(heigh = 800)
      ...
      dplyr::summarise(
        ...
        ggplot2::ggplot() +
          ggplot2::geom_line(ggplot2::aes(x = MES_REF, y = Quantidade, group = DIA, color = DIA)) +
            ggplot2::scale_color_manual(name = "Dia da entrega", values = cordegrade(7)) +
              ggplot2::scale_x_date(date_labels = "%b", date_breaks = "1 month") +
                tema_semest_linha()) %>% 
  plotly::ggplotly()
